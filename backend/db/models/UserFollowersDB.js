...
discussionsRouter and trigger notifications for each user.  Returning  uuid for pusher triggers username and avatar for if we want this information displayed on the frontend along with user id. 
*/
return db("user_followers")
  .innerJoin("users", "user_followers.user_id", "users.id")
  .where({ "user_followers.following_id" : following_id  })
  .innerJoin("user_settings", "users.id", "user_settings.user_id")
  .select(["user_followers.user_id", "uuid", "username", "user_settings.avatar" ])
};
