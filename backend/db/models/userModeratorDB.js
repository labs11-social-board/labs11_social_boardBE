	...
	return db('user_settings')
		...
		.then((data) => {
			...
			if (data[0].user_permissions != 'moderator') {
				return db('user_settings').where({ user_id: changeUser_id }).update({ user_permissions: 'moderator' });
			} else {
	...
	return db('user_settings')
		...
		.then((data) => {
			...
			if (data.user_permissions != 'basic') {
				return db('user_settings').where({ user_id: changeUser_id }).update({ user_permissions: 'basic' });
			} else {
